...
bot.hears('/restart', async (ctx) => {
    ...
    if (userId !== undefined ) {
        exec('pm2 restart notification_bot', (error: Error | null, _stdout: string, stderr: string) => {
            if (error) {
...
bot.command('status', async (ctx: Context) => {
    ...
    if (userId !== undefined) {
        try {
            const { stdout, stderr }: ExecResult = await execPromise('pm2 describe notification_bot');

import {BankData, BankApiResponse, BankApiItem} from './config.js';
...
function parseBanksData(apiResponse: BankApiResponse): BankData[] {
    if (!apiResponse.success) {
        throw new Error('API вернул ошибку');
    }

    return apiResponse.items.map((item: BankApiItem) => ({
        name: item.bank_name,
